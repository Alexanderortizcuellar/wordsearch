...
import v1.ui_batch as ui_batch
import v1.ui_export as ui_export
import v1.ui_words as ui_words
import v1.wordsearch_ui as wordsearch_ui
from wordsearch.wordsearch import WordSearch
from utilities.utilities import (
    clean_words,
    fill,
    remove_asterisks,
    predict_width_height,
)
from utilities import solver
import v1.solver_ui as solver_ui
...
class Solver(QDialog, solver_ui.Ui_Form):
    ...
    def solve(self):
        ...
        locations = []
        for word in words:
            loc = solver.find_word(word, grid)
            if len(loc) > 0:
...
class ImportWords(QDialog, ui_words.Ui_words):
    ...
    def load_words(self):
        words = [
            self.words_list.item(i, 0)
            for i in range(50)
            if self.words_list.item(i, 0) is not None
        ]
            ...
            if len(words) >= 1:
                ...
                if len(words[0]) > 1:
                    QMessageBox.warning(
                        self,
                        "Import words",
                        "Csv file must have one column, please adjust",
                    )
                    return
                else:
...
class Export(QDialog, ui_export.Ui_Dialog):
    def __init__(self, parent: QWidget) -> None:
        ...
        self.setupUi(self)
        self.parent = parent
        self.export_list.itemClicked.connect(
            lambda x: self.export_ui_widget.setCurrentIndex(
                self.export_list.indexFromItem(x).row()
            )
        )
        self.save_btn_txt.clicked.connect(lambda kind: self.get_file_name("txt"))
        self.save_btn_csv.clicked.connect(lambda kind: self.get_file_name("csv"))
        self.save_btn_xl.clicked.connect(lambda kind: self.get_file_name("xl"))
        self.save_btn_pdf.clicked.connect(lambda kind: self.get_file_name("pdf"))
        self.save_btn_json.clicked.connect(lambda kind: self.get_file_name("json"))
        self.save_btn_word.clicked.connect(lambda kind: self.get_file_name("word"))
        self.save_btn_html.clicked.connect(lambda kind: self.get_file_name("html"))
        ...
        if line.text() != "":
            if line.text().lower().endswith(kind):
                ...
                QMessageBox.warning(
                    self,
                    "Export wordsearch",
                    f"The file your trying to save needs to be of type {kind}",
                )
                return False
        else:
            QMessageBox.warning(self, "Export wordsearch", "please enter a filename")
            return False

    def get_file_name(self, kind):
        filters = {
            "csv": " CSV (*.csv)",
            "xl": "XLSX (*.xlsx);;XLSM (*.xlsm)",
            "pdf": "PDF (*.pdf)",
            "json": "JSON (*.json)",
            "word": "DOCX (*.docx)",
            "html": "HTML (*.html)",
    ...
    def save_text(self):
        try:
            if self.verify_file(self.filename_txt, "txt"):
                ...
                QMessageBox.about(
                    self, "Export wordsearch", f"file successfully saved as {file}"
                )
                if self.open_file_txt.isChecked():
                    os.startfile(file)
        except Exception as e:
            QMessageBox.warning(self, "Save Wordsearch", str(e))
...
class Window(QMainWindow, wordsearch_ui.Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.rows.valueChanged.connect(lambda x: self.number_rows.setText(str(x)))
        self.columns.valueChanged.connect(lambda x: self.number_columns.setText(str(x)))
    ...
    def generate(self):
        try:
            words = [
                self.words_list.item(i).text() for i in range(self.words_list.count())
            ]
            ...
            self.number_words.setText(str(self.wordsearch.output["number-words"]))
            ...
            self.number_letters.setText(str(self.wordsearch.output["number-letters"]))
            ...
            self.creation_time.setText(str(self.wordsearch.output["time-created"]))
            ...
            if self.wordsearch.output["number-words-no-placed"] >= 1:
                msg = QMessageBox.warning(
                    ...
                    + "\n".join(self.wordsearch.output["words-not-placed"]),
    ...
    def show_answers_func(self):
        if self.wordsearch is not None:
            current_grid = (
                remove_asterisks(deepcopy(self.wordsearch.grid))
                if self.show_answers.isChecked()
    ...
    def add_word(self):
        word, ok = QInputDialog.getText(self, "Wordsearch Words", "Enter New Word")
        if ok:
            if word != "":
    ...
    def edit_word(self):
        ...
        if word is not None:
            ...
            QMessageBox.warning(
                self, "Wordsearch words", "select the word you wanna edit"
            )
    ...
    def remove_word(self):
        ...
        if word >= 0:
            ...
            QMessageBox.warning(
                self, "Wordsearch words", "select the word you wanna remove"
            )
    ...
    def export_words(self):
        if self.words_list.count() >= 1:
            file, kind = QFileDialog.getSaveFileName(self, "Save", None, "CSV (*.csv)")
            if file:
                words = [
                    self.words_list.item(row).text()
